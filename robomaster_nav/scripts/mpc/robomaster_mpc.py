"""
RoboMaster S1 Spline Tracking MPC using ACADOS.

Spline tracking via alongtrack + crosstrack residuals, and soft ellipsoidal obstacle avoidance.
"""
...
from robomaster_dynamics import export_robomaster_model
...
# Ellipse defining robomaster footprint (semi-axes, metres)
A_ELLIPSE = 0.2  # longitudinal
B_ELLIPSE = 0.2  # lateral
ELLIPSE_OFFSET = 0.0  # shift centre fore/aft if COG isn't midship
R_SAFE_ELLIPSE = 0.3  # extra buffer added to both axes
...
def _build_residuals(model, terminal: bool):
    ...
    # Controls
    u_v0, u_v1, u_v2, u_v3 = model.u[0], model.u[1], model.u[2], model.u[3]
    ...
    if not terminal:
        ...
        pieces += [
            ca.sqrt(w_input) * u_v0,
            ca.sqrt(w_input) * u_v1,
            ca.sqrt(w_input) * u_v2,
            ca.sqrt(w_input) * u_v3,
        ]
...
def setup_spline_tracking_ocp(x0, params, Tf, N_horizon) -> AcadosOcpSolver:
    ...
    ocp.constraints.lbu = np.array(
        [-model.v_max, -model.v_max, -model.v_max, -model.v_max, DT_MIN]
    )
    ocp.constraints.ubu = np.array(
        [model.v_max, model.v_max, model.v_max, model.v_max, DT_MAX]
    )
    ...
    ocp.code_export_directory = "c_generated_code_robomaster_ocp"
    return AcadosOcpSolver(ocp, json_file="robomaster_ocp.json")


def setup_integrator(dt, params) -> AcadosSimSolver:
    """Closed-loop integrator for the simulation."""
    sim = AcadosSim()
    sim.model = export_robomaster_model()
    sim.solver_options.T = dt
    sim.solver_options.num_steps = 10
    sim.code_export_directory = "c_generated_code_robomaster_sim"
...
class Scenario:
    ...
    obs_bounds: tuple = (-5.0, 20.0, -10.0, 10.0)
    ...
    def initial_state(self) -> np.ndarray:
        ...
        return np.concatenate([np.array(self.robomaster0), flat_obs])
...
# Visualization
# =============================================================================
def _robomaster_verts(x, y, psi):
    """Rotate and translate the ROBOMASTER footprint to world coordinates."""
    cp, sp = np.cos(psi), np.sin(psi)
    R = np.array([[cp, -sp], [sp, cp]])
    return (ROBOMASTER_SHAPE @ R.T) + np.array([x, y])
...
class SimPlotter:
    ...
    def _setup_trajectory(self, ax):
        ...
        for i in range(OBS_N):
            ...
            circle = plt.Circle((ox, oy), 0.3, color="red", alpha=0.3)
            ax.add_patch(circle)
        ...
        (self._lines["traj"],) = ax.plot(
            [], [], "r-", linewidth=1.5, label="ROBOMASTER trajectory"
        )
        (self._lines["s_t"],) = ax.plot(
            [], [], "b^", markersize=10, zorder=6, label="s(t)"
        )
        self._artists["robomaster"] = Polygon(
            _robomaster_verts(x0[0], x0[1], x0[2]),
            closed=True,
            fc="green",
            ec="black",
            lw=1.5,
        ...
        ax.add_patch(self._artists["robomaster"])
        ax.legend()
    ...
    def _setup_controls(self, ax):
        ...
        ax.grid(True)
        (self._lines["v0"],) = ax.plot([], [], label="$v0$")
        (self._lines["v1"],) = ax.plot([], [], label="$v1$")
        (self._lines["v2"],) = ax.plot([], [], label="$v2$")
        (self._lines["v3"],) = ax.plot([], [], label="$v3$")
        ax.legend()
    ...
    def update(self, step, simX, simU, t_axis, t_axis_u, cost_hist):
        ...
        self._artists["robomaster"].set_xy(_robomaster_verts(x[0], x[1], x[2]))
        ...
        self._lines["v0"].set_data(t_axis_u, simU[: step + 1, 0])
        self._lines["v1"].set_data(t_axis_u, simU[: step + 1, 1])
        self._lines["v2"].set_data(t_axis_u, simU[: step + 1, 2])
        self._lines["v3"].set_data(t_axis_u, simU[: step + 1, 3])
...
def report_solver_failure(step, status, ocp_solver, simX, params, scenario):
    ...
    # Current ROBOMASTER state
    x, y, psi, surge = simX[step, 0], simX[step, 1], simX[step, 2], simX[step, 3]
    print(f"  ROBOMASTER: pos=({x:.1f},{y:.1f}) surge={surge:.2f} psi={psi:.3f} rad")
...
def run_simulation(scenario: Scenario):
    ...
    Nsim = scenario.Nsim
    nx = x0.shape[0]
    nu = 5  # 4 wheel commands (v0,v1,v2,v3) + 1 slack
...
def main():
    # CLI: `python robomaster_mpc.py [simulate]`  — simulate defaults to True.
    simulate = True
    if len(sys.argv) > 1:
