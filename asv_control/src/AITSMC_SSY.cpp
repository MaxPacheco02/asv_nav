...
Eigen::Vector3d AITSMC_SSY::compute_tau(const State &state,
  ...
  // ANTI-WINDUP
  Eigen::Vector3d e_i_dot_int = e_i_dot;
  for (int i = 0; i < 3; i++)
    if (e_i_dot_int(i) * saturation(i) > 0)
      e_i_dot_int(i) = 0.0;

  // SLIDING SURFACE
  // s = e + alpha*e_I
  e_i = integral_step * (e_i_dot_int + e_i_dot_last) / 2 + e_i;
  e_i_dot_last = e_i_dot_int;
