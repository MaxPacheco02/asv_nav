...
CatmulRom::CatmulRom(Eigen::Vector2d p0, Eigen::Vector2d p1, Eigen::Vector2d p2,
                     Eigen::Vector2d p3)
    : CatmulRom() {
  update(p0, p1, p2, p3);
}

void CatmulRom::update(Eigen::Vector2d p0, Eigen::Vector2d p1,
...
void CatmulRom::set_coeffs(Eigen::Vector2d a, Eigen::Vector2d b,
                           Eigen::Vector2d c, Eigen::Vector2d d) {
  s_.a = a;
  s_.b = b;
  s_.c = c;
  s_.d = d;
...
Eigen::Vector2d CatmulRom::get_s(double t) {
  return s_.a * t * t * t + s_.b * t * t + s_.c * t + s_.d;
}

Eigen::Vector2d CatmulRom::get_s_dot(double t) {
  return 3 * s_.a * t * t + 2 * s_.b * t + s_.c;
}
...
double CatmulRom::closest_t(Eigen::Vector3d p3) {
  ...
  // = (3a^2)t^5+(5ab)t^4+(4ac+2b^2)t^3+(3bc+3a(d-p))t^2+(c^2+2b(d-p))t+c(d-p) =
  // 0

  // Formulate 5th degree polynomial (coefficients from lowest to highest
  // degree)
  Eigen::VectorXd coefficients(6);
  coefficients << s_.c.dot(s_.d - p), s_.c.dot(s_.c) + 2 * s_.b.dot(s_.d - p),
      3 * s_.b.dot(s_.c) + 3 * s_.a.dot(s_.d - p),
      4 * s_.a.dot(s_.c) + 2 * s_.b.dot(s_.b), 5 * s_.a.dot(s_.b),
      3 * s_.a.dot(s_.a);

  // Get roots
  Eigen::PolynomialSolver<double, Eigen::Dynamic> solver;
  solver.compute(coefficients);
  const Eigen::PolynomialSolver<double, Eigen::Dynamic>::RootsType &r =
      solver.roots();
...
void CatmulRom::calc_arc_length() { L_ = get_arc_length(0.0, 1.0); }
